"""MANUAL live-broker probe for PyQuotexProvider -- har API call check karega.

NOT a unit test. This was previously `backend/test_market.py`, which meant
pytest collected it on every run even though it (a) imported
`app.core.market` -- a module that no longer exists after the
`app/core/` -> `app/services/` restructure, so collection errored -- and
(b) places a REAL $1 order against the broker when credentials are present.

Moved here and renamed so pytest never collects it, and credentials now come
from the environment instead of edited-in source constants.

Run:  cd backend && QUOTEX_EMAIL=... QUOTEX_PASSWORD=... \
          python scripts/manual_quotex_probe.py
"""
import asyncio
import logging
import os
...
# Add the backend package root to path
...
from app.services.market import PyQuotexProvider, PublicDataProvider  # noqa: E402
from app.schemas import Direction  # noqa: E402

# Back-compat alias: the old name for the no-credentials paper provider.
SimulatedProvider = PublicDataProvider
...
async def main():
    ...
    if not EMAIL or not PASSWORD:
        print("\n⚠️  Real provider ke liye QUOTEX_EMAIL / QUOTEX_PASSWORD export karo")
        return
