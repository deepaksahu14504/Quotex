...
# How long to wait for the broker to ACKNOWLEDGE a buy. This is a websocket
# round trip, not the life of the option -- see the note in buy() below.
BUY_CONFIRM_TIMEOUT: int = DEFAULT_TIMEOUT
# Bound for the pre-order preparation calls, which must fail before anything
# is transmitted rather than after.
...
class TradingMixin:
    ...
    async def buy(
        ...
        #
        # BUG FIX: these three preparation calls were unbounded. Each one waits
        # on the same websocket that has just gone quiet in the failure case
        # this method is trying to survive, so a stalled socket wedged buy()
        # before the order request was even sent -- and the caller had no way
        # to tell "we never asked the broker" from "we asked and heard
        # nothing", which are very different states when real money is
        # involved. Bounded here so a wedged socket fails FAST and, crucially,
        # fails BEFORE any order is transmitted.
        try:
            await asyncio.wait_for(
                self.start_realtime_price(asset, duration), timeout=PREPARE_TIMEOUT)
            await asyncio.wait_for(self.get_server_time(), timeout=PREPARE_TIMEOUT)
            await asyncio.wait_for(
                self.api.settings_apply(asset, duration, is_fast_option),
                timeout=PREPARE_TIMEOUT)
        except asyncio.TimeoutError:
            logger.error(
                "Timed out preparing the order for %s (socket unresponsive). "
                "No order was sent.", asset)
            return False, "Timeout preparing order — no order was sent"

        await self.api.buy(
            amount, asset, direction, duration, request_id, is_fast_option, time_mode
        )

        # BUG FIX: this was `duration + 5`, which conflates two unrelated
        # things -- how long the OPTION runs, and how long the broker takes to
        # ACKNOWLEDGE that the order was accepted. The acknowledgement is a
        # websocket round trip: it arrives in well under a second on a healthy
        # connection, or it never arrives at all. Tying it to duration meant a
        # 15m trade would sit here for 905 seconds waiting for a confirmation
        # that was never coming, and even a 1m trade waited 65s. Callers that
        # imposed their own (much shorter) execution timeout therefore saw
        # every wedged order as "did not return in Ns" with the underlying
        # call still parked in this wait.
        #
        # A fixed acknowledgement budget is the correct semantics. It is NOT
        # reduced below DEFAULT_TIMEOUT, so a merely slow-but-alive connection
        # still succeeds.
