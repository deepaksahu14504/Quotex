...
class QuotexAPI:
    ...
    def __init__(
        ...
        # PRODUCTION FIX (candle-drop race): _temp_status used to be a single
        # shared string set by *any* "451-[...]" placeholder frame and
        # cleared after the *next* data frame, regardless of whether that
        # frame actually belonged to it. If a second placeholder-style event
        # (e.g. instruments/list, sent on every reconnect and by periodic
        # asset-list refreshes) arrived while a history/load request was
        # still in flight, the real candle payload got silently routed into
        # the wrong branch and dropped -- see repro_race.py.
        #
        # Socket.IO delivers frames strictly in order over one connection,
        # so the Nth placeholder frame always correlates with the Nth data
        # frame that follows it -- a FIFO queue preserves that pairing even
        # when different placeholder-style event types interleave, which a
        # single shared string cannot. _temp_status itself is kept (last
        # popped value) purely for logging/back-compat; it is NOT used for
        # routing decisions anymore.
        self._temp_status: str = ""
        self._temp_status_queue: "deque[str]" = deque()
        ...
        # TEMP CANDLE-TRACE LOGGING (dispatcher registration) — remove once
        # the history/load placeholder-misrouting investigation is closed.
        logger.debug(
            "[CANDLE_TRACE] dispatcher registered: instance_id=%s handlers=%s",
            id(self), sorted(self._control_handlers.keys()),
        )
    ...
    def _track_subscription(
        ...
        **extra: Any,
        ...
        logger.debug(
            "[CANDLE_TRACE] subscription tracked: instance_id=%s key=%s total=%d",
            id(self), key, len(self._subscriptions),
        )
    ...
    def _forget_subscription(
        ...
        logger.debug(
            "[CANDLE_TRACE] subscription forgotten: instance_id=%s key=%s",
            id(self), key,
        )
    ...
    @staticmethod
    def _describe_payload(data: Any, limit: int = 4) -> str:
        """Bounded, secret-free description of an instruments payload.

        Reports type, length, and the first few row types/lengths with a
        truncated sample of each row's first two positional values.
        Instrument symbols are not secrets; credentials, cookies, SSID and
        headers never appear in an instruments/list frame, and nothing
        here reaches into dict values beyond key names.
        """
        if isinstance(data, dict):
            return f"dict(keys={sorted(data)[:8]})"
        if not isinstance(data, (list, tuple)):
            return f"{type(data).__name__}({repr(data)[:60]})"
        head = []
        for r in list(data)[:limit]:
            if isinstance(r, (list, tuple)):
                sample = ", ".join(repr(v)[:20] for v in list(r)[:2])
                head.append(f"{type(r).__name__}(len={len(r)})[{sample}]")
            elif isinstance(r, dict):
                head.append(f"dict(keys={sorted(r)[:4]})")
            else:
                head.append(f"{type(r).__name__}({repr(r)[:20]})")
        return f"list(len={len(data)}) rows={head}"

    @staticmethod
    def _is_instrument_payload(data: Any) -> tuple[bool, str]:
        """Positive check that a payload really is an instrument list.

        The previous guard (`_looks_like_candles`) only inspected DICTS:

            _looks_like_candles = isinstance(data, dict) and (
                data.get("candles") ... or data.get("data") ... or data.get("asset") ...)

        A BARE LIST of candle rows -- `[[time, open, close, high, low], ...]`,
        which is exactly what a history/load response carries -- was never
        examined, fell through to `if isinstance(data, list): self.instruments
        = data`, and became the instrument list. Four candles then read as
        four rows of length 5, producing the production symptom
        "4/4 instrument rows failed validation" with zero assets parsed.

        It also LATCHED: _api/assets.py:29 returns early on
        `if self.api.instruments and len(...) > 0`, so the bad value was
        served as the cache and the real list was never re-requested.

        An instrument row carries a string symbol at index 1
        (_api/assets.py:69,113,130). A candle row carries a NUMBER there
        (the open price). That single positional difference separates them
        reliably without inventing any new index.
        """
        if isinstance(data, dict):
            if "list" in data:
                return QuotexAPI._is_instrument_payload(data["list"])
            return False, f"dict without a 'list' key (keys={sorted(data)[:8]})"
        if not isinstance(data, (list, tuple)):
            return False, f"payload is {type(data).__name__}, not a list"
        if not data:
            return False, "empty list"
        for r in data[:8]:
            if not isinstance(r, (list, tuple)) or len(r) < 2:
                return False, "rows are not positional instrument records"
            if isinstance(r[1], (int, float)) and not isinstance(r[1], bool):
                return False, ("rows carry a NUMBER at index 1 where a symbol "
                               "is expected -- this is a candle/quote payload")
        return True, ""
    ...
    async def _h_instruments_list(self, data: Any) -> None:
        if isinstance(data, dict) and data.get("_placeholder"):
            ...
            # ROOT CAUSE of the production symptom:
            #   Assets feed        : "Broker returned an empty instrument response"
            #   Instrument parsing : "4/4 instrument rows failed validation"
            #
            # This used to be a bare `self.instruments = data`, assigning
            # WHATEVER arrived -- including a dict. `_on_message`'s
            # instruments/list branch (line ~467) already validated the
            # shape properly:
            #     if isinstance(data, list):            -> use it
            #     elif isinstance(data, dict) and "list" -> use data["list"]
            # but this dispatch-table handler, the other writer to the same
            # attribute, had no check at all. Two writers, one validated,
            # one not.
            #
            # When a non-list payload landed here (a 4-key dict, e.g. a
            # candle-shaped frame delivered under this event), self.instruments
            # became that dict. Downstream, market.py::get_assets() does
            # `len(instruments)` -> 4 (key count) and `for i in instruments`
            # -> iterates the four KEY STRINGS, every one of which fails the
            # row validation. Hence exactly "4/4 rows failed" and zero parsed
            # assets, reported as an empty broker response.
            #
            # Worse, it persisted: _api/assets.py::get_instruments() returns
            # early on `if self.api.instruments and len(...) > 0`, and a
            # 4-key dict is truthy with len 4 -- so the bad value was served
            # as the cached instrument list and the real list was never
            # re-requested.
            #
            # Same validation as the _on_message branch. A payload that is
            # neither shape is ignored rather than stored: instruments_ready
            # does not fire, get_instruments() keeps waiting, and the last
            # good list stays intact.
            ok, why = self._is_instrument_payload(data)
            if not ok:
                logger.warning(
                    "INSTRUMENT_SCHEMA_DIAGNOSTIC source=instruments/list-handler "
                    "rejected=%s payload=%s -- keeping the previous instrument "
                    "list rather than storing an unusable value.",
                    why, self._describe_payload(data),
                )
                return
            self.instruments = data["list"] if isinstance(data, dict) else data
    ...
    async def _h_candle_generated(self, data: Any) -> None:
        ...
        if asset and period:
            logger.debug(
                "[CANDLE_TRACE] candle-generated push: instance_id=%s "
                "asset=%s period=%s",
                id(self), asset, period,
            )
    ...
    async def _on_open(self) -> None:
        ...
        logger.debug(
            "[CANDLE_TRACE] _on_open: instance_id=%s stale temp_status=%r "
            "stale queue_depth=%d stale subscriptions=%s (about to send "
            "instruments/get, which produces its own 451-[...] placeholder)",
            id(self), self._temp_status, len(self._temp_status_queue),
            list(self._subscriptions.keys()),
        )
        # PRODUCTION FIX: a fresh connection cannot receive the data frame
        # for a placeholder that was queued on the previous connection (that
        # request is simply gone) -- discard it rather than let it sit
        # forever and potentially pair with an unrelated frame from the new
        # connection.
        self._temp_status_queue.clear()
        self._temp_status = ""
        ...
        try:
            ...
            if is_control:
                if "51-" in msg_str and "_placeholder" in msg_str:
                    # TEMP CANDLE-TRACE LOGGING — this is the exact frame the
                    # UI/logs show as "451-[...]". It is step 1 of a normal
                    # 2-frame Socket.IO binary-event handshake: the real
                    # payload is a SEPARATE frame that arrives next. It is
                    # now correlated via a FIFO queue (production fix below),
                    # not a single shared string.
                    logger.debug(
                        "[CANDLE_TRACE] placeholder frame received: instance_id=%s "
                        "prev_temp_status=%r new_temp_status=%r queue_depth=%d",
                        id(self), self._temp_status, msg_str, len(self._temp_status_queue),
                    )
                    self._temp_status = msg_str
                    # PRODUCTION FIX: append rather than overwrite, so a
                    # second placeholder arriving before the first request's
                    # real payload does NOT erase the first request's tag.
                    self._temp_status_queue.append(msg_str)
                    return
            ...
            # 2. Handle Data Payloads (Placeholder fulfillment)
                ...
                # PRODUCTION FIX (candle-drop race): pop this frame's own
                # correlation tag from the FIFO queue instead of trusting
                # whatever self._temp_status happens to currently hold.
                # Socket.IO delivers frames strictly in order on one
                # connection, so the Nth placeholder always precedes the Nth
                # data frame -- popleft() here always retrieves the tag that
                # was pushed for *this* frame, even if other placeholder-style
                # events (instruments/list, orders/*, ...) were interleaved in
                # between. Falls back to the legacy self._temp_status only if
                # the queue is empty (e.g. a data frame with no preceding
                # placeholder was ever seen) -- never worse than the old
                # behavior.
                if self._temp_status_queue:
                    status = self._temp_status_queue.popleft()
                else:
                    status = self._temp_status
                self._temp_status = status  # kept in sync for logging/back-compat only

                # TEMP CANDLE-TRACE LOGGING — this is the "step 2" data frame
                # for whatever placeholder came before it. Routing below is
                # now decided by `status`, this frame's own correlated tag
                # (see the FIFO queue fix above) rather than a shared mutable
                # string that a later, unrelated placeholder could overwrite.
                _looks_like_candles = isinstance(data, dict) and (
                    data.get("candles") is not None
                    or data.get("data") is not None
                    or data.get("asset") is not None
                )
                logger.debug(
                    "[CANDLE_TRACE] data payload received: instance_id=%s "
                    "status=%r data_type=%s looks_like_candles=%s "
                    "data_keys=%s",
                    id(self), status, type(data).__name__,
                    _looks_like_candles,
                    list(data.keys()) if isinstance(data, dict) else None,
                )

                if status and 'instruments/list' in status:
                    # `_looks_like_candles` only ever inspected dicts, so a
                    # BARE LIST of candle rows was stored here unchallenged.
                    # _is_instrument_payload covers both shapes and REJECTS
                    # rather than warning-and-storing, which is what the old
                    # MISROUTE branch did.
                    _ok, _why = self._is_instrument_payload(data)
                    if not _ok:
                        logger.warning(
                            "INSTRUMENT_SCHEMA_DIAGNOSTIC source=_on_message "
                            "status=%r rejected=%s payload=%s -- previous "
                            "instrument list preserved.",
                            status, _why, self._describe_payload(data),
                        )
                    else:
                        self.instruments = (
                            data["list"] if isinstance(data, dict) else data
                        )

                    if _ok and self.instruments:
                        await self.event_registry.set_event(
                            'instruments_ready', self.instruments
                        )

                elif (
                        any(x in status for x in ['history/list/v2', 'history/load'])
                    ...
                    if isinstance(data, dict) and data.get("asset"):
                        ...
                        logger.debug(
                            "[CANDLE_TRACE] candle callback: instance_id=%s "
                            "asset=%s index=%s status_matched=%s -> "
                            "firing candles_ready_%s",
                            id(self), asset, data.get("index"),
                            any(x in status for x in ['history/list/v2', 'history/load']),
                            asset,
                        )
                        await self.event_registry.set_event(
                    ...
                    if (
                            any(x in status for x in ['orders/open', 'pending/create'])
                ...
                logger.debug(
                    "[CANDLE_TRACE] clearing temp_status: instance_id=%s "
                    "was=%r queue_depth=%d",
                    id(self), self._temp_status, len(self._temp_status_queue),
                )
                self._temp_status = ""  # legacy attribute, cleared for back-compat
    ...
    #: Ceiling on a single WebSocket send. A healthy send completes in
    #: single-digit milliseconds; anything approaching this means the
    #: transport is gone, not that the broker is busy.
    WS_SEND_TIMEOUT = 10.0
    #: Ceiling on waiting for the send lock. Deliberately larger than
    #: WS_SEND_TIMEOUT so a caller queued behind ONE slow-but-alive send
    #: still gets its turn, while a caller queued behind a genuinely dead
    #: send gives up shortly after that send has itself timed out.
    WS_SEND_LOCK_TIMEOUT = 15.0
    ...
    async def send_websocket_request(self, data: str) -> None:
        ...
        Raises:
            TimeoutError: the transport did not accept the frame in time,
                or the send lock could not be acquired because another
                sender is wedged on a dead transport.

        ROOT-CAUSE FIX (pipeline stops making progress shortly after
        startup). This method previously read:

            async with self._ws_send_lock:
                if self.websocket:
                    await self.websocket.send(data)

        Neither operation was bounded, and that is the single point every
        socket-touching call in the whole application funnels through.

        When the TCP connection goes half-open -- the peer, a NAT table,
        or a firewall drops it silently with no FIN and no RST, which is
        routine for a long-lived broker socket -- `websocket.send()` does
        NOT raise. It writes into the OS send buffer and, once that fills
        with no ACKs coming back, awaits drain forever.

        The consequences cascaded, and each step is reproduced in
        backend/prove_root_cause.py:

          1. The stuck sender holds `_ws_send_lock` for the whole
             duration of the `async with` body, i.e. forever.
          2. Every OTHER caller -- start_candles_stream, get_candles,
             get_historical_candles, place_order -- then blocks on the
             lock. The socket becomes permanently unusable. Cancelling a
             waiter does not help: the lock is held by the first task,
             not by the waiters.
          3. Orchestrator._evaluate_asset_signal blocks inside
             provider.get_candles, so _scan_once's asyncio.gather() --
             which waits for ALL candidates -- never returns.
             `return_exceptions=True` does not save it; a hang is not an
             exception.
          4. _scan_loop therefore never reaches
             `health["last_scan_at"] = now_ts()` and never starts its
             next iteration.
          5. Nothing detected it. TaskSupervisor only knows Task.done()
             is False, so it reported the loop as healthy with zero
             restarts. The watchdog checks `provider.connected`, a bool
             set at connect time that a half-open socket never changes.
             And _scan_loop's one bounded call, get_balance, reads
             pyquotex's CACHED account_balance (see
             _api/account.py:136) -- it never touches the socket, so its
             wait_for never fired.

        Bounding both operations converts an invisible permanent hang
        into an ordinary TimeoutError, which every caller in this
        codebase already handles, and which the reconnect path already
        knows how to act on. The status flip below is what actually
        restarts the transport: without it a bounded send would just
        fail forever on the same dead socket.
        """
        try:
            await asyncio.wait_for(
                self._ws_send_lock.acquire(), timeout=self.WS_SEND_LOCK_TIMEOUT
            )
        except asyncio.TimeoutError:
            # Someone ahead of us is wedged on a dead transport. Fail
            # fast rather than joining the queue behind them forever.
            self._mark_transport_failed("send lock acquire timed out")
            raise TimeoutError(
                f"WebSocket send lock not acquired within "
                f"{self.WS_SEND_LOCK_TIMEOUT}s -- transport considered dead"
            )
        try:
            ...
            try:
                await asyncio.wait_for(
                    self.websocket.send(data), timeout=self.WS_SEND_TIMEOUT
                )
            except asyncio.TimeoutError:
                self._mark_transport_failed("send() timed out")
                raise TimeoutError(
                    f"WebSocket send did not complete within "
                    f"{self.WS_SEND_TIMEOUT}s -- transport considered dead"
                )
        finally:
    ...
    def _mark_transport_failed(self, reason: str) -> None:
        ...
        Without this, bounding the send would only convert a permanent
        hang into a permanent stream of TimeoutErrors against the same
        dead socket. Flipping status to ERROR is what
        Orchestrator._watchdog_loop's `not provider.connected` check
        reads, so auto-reconnect finally fires -- which is exactly what
        it was written to do and never got the chance to.

        Deliberately synchronous and allocation-free: it is called from
        inside an except block on a path that is already degraded.
        """
        if self.state.status == WebsocketStatus.ERROR:
            return  # already flagged; don't spam
        logger.error(
            "WebSocket transport declared dead (%s). Marking ERROR so the "
            "reconnect path engages; every queued sender will now fail fast "
            "instead of blocking forever.", reason,
        )
        self.state.status = WebsocketStatus.ERROR
