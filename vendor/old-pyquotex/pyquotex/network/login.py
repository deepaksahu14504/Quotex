...
class Login(Browser):
    ...
    async def awaiting_pin(
        ...
        # BUG FIX: this result used to be discarded, leaving self.response
        # pointing at the pre-OTP sign-in page. success_login() checks
        # self.response.url for "trade" to decide success — with the old
        # code that was always the PRE-otp page, which never contains
        # "trade", so login was reported as failed ("Unknown error") even
        # when the OTP code was entered correctly and Quotex's own response
        # to this exact POST redirected to the trade page.
    ...
    async def success_login(self) -> tuple[bool, str]:
        ...
        if not error:
            # DIAGNOSTIC: the expected error element wasn't on the page at all,
            # meaning Quotex served something our selector doesn't recognize
            # (captcha, "confirm this device" screen, rate-limit block, a
            # changed page layout, etc). Log enough to identify which, without
            # dumping the whole page (could contain the session cookie value
            # inline in some Quotex builds).
            title = soup.title.get_text(strip=True) if soup.title else "(no title)"
            body_text = soup.get_text(" ", strip=True)[:400]
            print(f"[LOGIN DIAGNOSTIC] Unknown error — response_url={response_url!r} "
                  f"page_title={title!r} body_preview={body_text!r}")
